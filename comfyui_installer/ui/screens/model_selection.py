...
class ModelSelectionScreen(Screen):
    ...
    BINDINGS = [
        ...
        Binding("enter", "next_step",       "Next: Quality", priority=True),
    ...
    def on_mount(self) -> None:
        # Restore previously selected families from app state
        for fid in self.app.selected_models:
    ...
    def _label(self, family: ModelFamily) -> str:
        check = "[bold green]\\[✓][/] " if self._selected[family.id] else "[dim]\\[ ][/] "
    ...
    def _on_quality_chosen(self, result: dict[int, int] | None) -> None:
        if result is not None:
            ...
            main = next((s for s in self.app.screen_stack if isinstance(s, MainMenuScreen)), None)
            if main is not None:
                ...
                names = [f.name for f in MODEL_FAMILIES if f.id in result]
                main.selected_model_names = ", ".join(names)
                main.last_action = f"Selected {len(result)} model famil{'y' if len(result) == 1 else 'ies'}."
...
class QualitySelectionScreen(Screen):
    ...
    def compose(self) -> ComposeResult:
        ...
        items = [
            ListItem(Label(f" {idx+1}. {name}"), id=f"tier_{idx}")
